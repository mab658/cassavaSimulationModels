...
# Take average of output from Mega environment 1 and 2 as narrow breeding program
result.me <- rbind(result.me1,result.me2)

result.narrow <- result.me %>%
	dplyr::group_by(simRun,year,scenario) %>%
  	summarize_all(mean, na.rm = TRUE)


# combine data	from 3 scenarios
simData <- rbind(result.conv,result.broad,result.narrow)
...
simSumm <- simData %>%
	...
	selAccur= mean(accCET),
    	seAccur = sd(accCET)/sqrt(length(simRun)),.groups = 'drop')
...
# plot line trends of the accuracy at CET stage
