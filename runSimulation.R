...
mc.cores <- 70  # initially set to 15 (good) for 25 runs
...
cat("\n Summary of genetic psrameters from last year of burnin to last yesr of delection cycle \n")
print(simSumm)

# plot of genetic mean
...
print(str(regData))
print(levels(regData$scenario))
  regData$scenario <- factor(regData$scenario,
                              levels = c("Conventional","Broad adaptation",
                                         "Narrow adaptation"),
                              labels=c("Conventional","Broad adaptation",
                                       "Narrow adaptation"))

  rateGenGain <- ggplot(regData, aes(x = year, y = genMean,color=scenario)) +
