...
#print(selAccPlot)
 ...
 # save the plot to a file
ggsave("./output/selAccuracyMean.jpeg",height = 4.5, width = 6.5, units = "in", dpi = 300)
...
# save the plot to a file
ggsave("./output/selectionAccuracyTrend.jpeg",height = 4.5, width = 6.5, units = "in", dpi = 300)


# boxplot comparing the distribution of  heritability estimate accuracy across the breeding scnearui the stages

distH2 <- simData %>%
  dplyr::filter(year>0) %>%
dplyr::select(simRun, year,scenario,H2)

distH2$scenario <- factor(distH2$scenario,
                                levels = c("Conv","Broad adaptation",
                                      "Narrow adaptation"),
                                labels=c("Conventional","Broad adaptation",
                                 "Narrow adaptation"))
...
distH2 <- ggplot(data = distH2, aes(x = scenario,y = H2, fill = scenario)) +
    geom_boxplot( stat = "boxplot",  outlier.colour = "red", outlier.shape = 16,
        outlier.size = 0.5, na.rm=TRUE) +
  labs(x= "Breeding strategy", y= "Heritability") + theme_bw() +
  theme(axis.title = element_text(colour="black", size=12),
         plot.title = element_text(hjust = 0.5,lineheight = .5,colour="black", size = 12),
         axis.text = element_text(face = "bold", size = 7))
...
# save the plot to a file
ggsave("./output/distHeritability.jpeg",height = 4.5, width = 6.5, units = "in", dpi = 300)
...
H2Plot <- ggplot(data = meanH2,aes(x = scenario,y = H2,fill = scenario)) +
geom_bar(stat = "identity", position = "dodge")+
  geom_text(aes(x = scenario,y = H2,label = round(H2,2)),
            position = position_dodge(width = 0.9),vjust=-0.25)+
  labs(x = "Breeding programme", y = "Heritability estimate")

#print(H2Plot)

 # save the plot to a file
ggsave("./output/H2MeanSummary.jpeg",height = 4.5, width = 6.5, units = "in", dpi = 300)
