...
# placeholder to  track the progress of breeding program
# by initializing variables to save the mean and variance of genetic values
simRun <- meanGV <- varGV <-  vector("numeric",length=nCycles)

h2 <- H2 <-  vector("numeric",length=nCycles) # track narrow-sense and broad-sense heritability
...
# create a placeholder to store p-value range to denote environmental covariate for each location
 	...
 	# records. The number of years retained is set with "limityear" in GlobalParameters.
  	...
  	if(year == startTrainPop){
		...
		# genotyping data of TP for ME1    
    		...
    		# update phenotype and genotype for ME1
   		...
   		# update phenotype and genotype for ME2
