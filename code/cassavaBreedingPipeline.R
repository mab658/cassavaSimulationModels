...
for (year in 1:7){
  ...
  if (year < 7){
    SDN <- setPheno(pop = F1, varE = errVarSDN,reps = repSDN,p = P[1],simParam = SP)
    parents <- selectWithinFam(pop=SDN, nInd=1, use="pheno", simParam=SP)
